...
#	push(@output_table,$sample."\t0\t0");
	...
	print STDOUT $sample,"\t","0","\t","0","\t",0,"\n";
	for(my $subRD=$BY;$subRD <= $RD;$subRD+=$BY) {
		for my $iter (1 .. $REP){
#my $Sys_cmd=$samtools_PATH." merge - ".$BAMs." | ".
#	$samtools_PATH." bamshuf -Ou - ".$tmp_DIR."/samtools_tmpR | ".
#	$samtools_PATH." view - | head -n ".$subRD;
# NOTE: I tried bamshuf function by samtools and performed worse than shuf
#my $Sys_cmd=$samtools_PATH." merge - ".$BAMs." | ".
#	$samtools_PATH." view - | shuf | head -n ".$subRD;
		
		# Check if subRD is greater than total reads
		if($subRD >  $Nreads) {
			print STDOUT $sample,"\t",$subRD,"\t","NA","\n";
		}
...
#		push(@output_table,$sample."\t".$subRD."\t".$cnt);
		print STDOUT $sample,"\t",$subRD,"\t",$cpm_cnt,"\t",$counts_cnt,"\n";
